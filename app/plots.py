...
def create_custom_visualisation(area, plot_data, y_variables=data.model_vars, y_definition=data.label_def_ordered,
				y_extra_info=data.label_extra_ordered, div_name="myplot"):
	values = plot_data.to_numpy()
	values = values[0]
	...
	plot = figure(plot_height=600, plot_width=800,
				  x_axis_label='Percentage',
				  # y_axis_label = ,
				  x_range=y_variables, y_range=(0, 100), tools=TOOLS, tooltips=tooltips)

	plot.scatter(x='variables',y='values',size=10,color='blue', source=all_data)
	# plot.hbar(left='variables', y= 'values', right=1, height=0.9, fill_color='blue', line_color='black', fill_alpha=0.75,
	# 		  hover_fill_alpha=1.0, hover_fill_color='navy', source=all_data)
	plot.title.text = "Relevant statistics about " + area
